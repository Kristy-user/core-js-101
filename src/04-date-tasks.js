...
function timeSpanToString(startDate, endDate) {
  return `${(endDate.getHours() - startDate.getHours())
    .toString()
    .padStart(2, '0')}:${(endDate.getMinutes() - startDate.getMinutes())
    .toString()
    .padStart(2, '0')}:${(endDate.getSeconds() - startDate.getSeconds())
    .toString()
    .padStart(2, '0')}.${(
...
function angleBetweenClockHands(date) {
  ...
  const angleRad = (Math.PI * Math.abs(60 * h - 11 * min)) / 360;
  return angleRad > Math.PI ? angleRad - Math.PI : angleRad;
}
